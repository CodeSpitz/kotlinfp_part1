...
fun <A> Stream<A>.tolist(): List<A> {
    tailrec fun go(xs: Stream<A>, acc: List<A>): List<A> = when (xs) {
        is Empty -> acc
        is Cons -> go(xs.tail(), ConsL(xs.head(), асс))
    }
    return reverse(go(this, Nill))
}


@DisplayName("연습문제 5.2")
fun <A> Stream<A>.take(n: Int): Stream<A> {
    fun go(xs: Stream<A>, n: Int): Stream<A> =
        when (xs) {
            is Empty -> empty()
            is Cons ->
                if (n == 0) empty()
...
@DisplayName("연습문제 5.4")
fun <A> Stream<A>.forAll(p: (A) -> Boolean): Boolean =
    foldRight({ true }, { a, b -> p(a) && b() })

@DisplayName("연습문제 5.5")
fun <A> Stream<A>.takeWhile(p: (A) -> Boolean): Stream<A> =
    foldRight({ empty() },
        { h, t -> if (p(h)) cons({ h }, t) else t() })

@DisplayName("연습문제 5.6")
fun <A> Stream<A>.headOption(): Option<A> =
    this.foldRight(
        { Option.empty() }, { a, _ -> Some(a) }
    )

@DisplayName("연습문제 5.7")
...
@DisplayName("연습문제 5.8")
fun <A> constant(a: A): Stream<A> =
    Stream.cons({ a }, { constant(a) })

@DisplayName("연습문제 5.9")
fun from(n: Int): Stream<Int> =
    cons({ n }, { from(n + 1) })

@DisplayName("연습문제 5.10")
fun fibs(): Stream<Int> {
